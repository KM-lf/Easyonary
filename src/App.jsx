...
// Get word of the day — changes daily, consistent for all users
...
async function callModel(word, model) {
...
async function lookupWord(word) {
  ...
  for (const model of models) {
    try {
      const result = await callModel(word, model);
      return result;
    } catch (e) {
  ...
  const search = async (word) => {
    ...
    try {
      ...
      if (e.message === "RATE_LIMIT") {
        // Calculate midnight UTC+7 (WIB)
        const now = new Date();
        const tomorrow = new Date(now);
        tomorrow.setUTCHours(17, 0, 0, 0); // 17:00 UTC = 00:00 WIB next day
        if (now.getUTCHours() >= 17) tomorrow.setUTCDate(tomorrow.getUTCDate() + 1);
        const hours = tomorrow.getUTCHours() + 7;
        const resetHour = hours >= 24 ? hours - 24 : hours;
        setError(`😴 Daily limit reached! Come back tomorrow at ${resetHour}:00 WIB and we'll be ready again!`);
      } else {
        setError("Couldn't find that word, please try again!");
      }
    } finally {
      setLoading(false);
    }
      ...
      <div style={styles.container}>
        ...
        {!result && !loading && !error && (
          <div style={styles.emptyState}>
            ...
            <div style={styles.wotdBox}>
              ...
              <button style={styles.wotdBtn} onClick={() => { setQuery(wotdWord); search(wotdWord); }}>
                Look it up →
              </button>
            </div>
        ...
        {loading && (
          <div style={styles.loadingWrap}>
            ...
            <div style={styles.loadingText}>Looking it up… (retrying if needed)</div>
          </div>
        )}
...
const styles = {
  ...
  wotdBox: {
    background: "linear-gradient(135deg, rgba(120,80,255,0.15), rgba(80,40,200,0.08))",
    border: "1.5px solid rgba(120,80,255,0.3)",
    borderRadius: "16px", padding: "24px", marginBottom: "32px",
    textAlign: "center",
  },
  wotdLabel: {
    fontSize: "11px", fontWeight: "700", color: "#b39dff",
    letterSpacing: "0.15em", marginBottom: "10px",
  },
  wotdWord: {
    fontFamily: "'Playfair Display', serif",
    fontSize: "32px", fontWeight: "700", color: "#f2f0ff",
    marginBottom: "16px", textTransform: "capitalize",
  },
  wotdBtn: {
    padding: "10px 24px",
    background: "linear-gradient(135deg, #7850ff, #5028c8)",
    border: "none", borderRadius: "999px",
    color: "#fff", fontWeight: "600", fontSize: "14px",
    fontFamily: "'DM Sans', sans-serif",
    boxShadow: "0 4px 16px rgba(120,80,255,0.3)",
  },
