...
const T = {
  en: {
    ...
    rateLimit: (h) => `😴 Daily limit reached! Come back tomorrow at ${h}:00 WIB and we'll be ready again!`,
  ...
  id: {
    ...
    rateLimit: (h) => `😴 Batas harian tercapai! Kembali besok jam ${h}:00 WIB ya!`,
...
async function callModel(word, model, lang) {
  ...
  const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: "user",
        content: `You are a dictionary assistant. ${langNote} For the word or phrase "${word}", return ONLY a JSON object with no markdown, no backticks, no extra text. Format:
{
...
export default function App() {
  ...
  const sendReport = () => {
    ...
    window.open(`mailto:report@easyonary.app?subject=${subject}&body=${body}`);
    setReportSent(true);
